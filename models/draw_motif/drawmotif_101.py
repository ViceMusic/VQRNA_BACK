...
from motifmodel.motifmodel_101 import Lucky
...
def load_model_and_data():
    ...
    model.load_state_dict(torch.load(f"../save/length/length101/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
for i in range(len(test_dataset)):
    ...
    X_attr2 = saturation_mutagenesis(wrapper.cpu(), x.cpu(), start=75 - 25, end=75 + 25, device='cpu')
    X_attr3 = saturation_mutagenesis(wrapper.cpu(), x.cpu(), start=0, end=100, device='cpu')
...
importance_score = torch.sum(avg_attr3, dim=1)
...
# 绘制第一个图
ax1 = plt.subplot(2, 1, 1)  # 2 行 1 列，选择第 1 个位置
plot_logo(avg_attr1[0, :, :], ax=ax1,color=custom_colors)  # 绘制特征重要性 logo 图
ax1.set_title('Midpoint of [0-50]:index=25', fontsize=50)

# 绘制第二个图
ax2 = plt.subplot(2, 1, 2)  # 2 行 1 列，选择第 2 个位置
plot_logo(avg_attr2[0, :, :], ax=ax2,color=custom_colors)  # 绘制特征重要性 logo 图
ax2.set_title('Midpoint of [50-100]:index=75', fontsize=50)
