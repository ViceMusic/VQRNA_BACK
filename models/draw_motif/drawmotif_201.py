...
from motifmodel.motifmodel_201 import Lucky
...
def load_model_and_data():
    ...
    model.load_state_dict(torch.load(f"../save/length/length201/data{params['data_index']}/seed{params['seed']}.pth")['state_dict'])
...
for i in range(len(test_dataset)):
    ...
    X_attr3 = saturation_mutagenesis(wrapper.cpu(), x.cpu(), start=0, end=200, device='cpu')
...
ax1.set_title('Midpoint of [0-100]:index=50', fontsize=50)

# 绘制第二个图
ax2 = plt.subplot(2, 1, 2)  # 2 行 1 列，选择第 2 个位置
plot_logo(avg_attr2[0, :, :], ax=ax2,color=custom_colors)  # 绘制特征重要性 logo 图
ax2.set_title('Midpoint of [100-200]:index=150', fontsize=50)
